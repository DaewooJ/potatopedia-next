 ...
 * Expects entries with { role: "user"|"assistant", text: string } — the internal
 * shape used by AskChat. Returns undefined when there's no usable history, so
 * callers can spread it into the body without polluting the request.
 */
export function buildHistory(messages) {
  ...
  const pairs = messages
    ...
    .map((m) => ({
      role: m.role,
      content: m.role === "assistant" ? m.text.slice(0, ASSISTANT_TRUNCATE) : m.text,
    }));
